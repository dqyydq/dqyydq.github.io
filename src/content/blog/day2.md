...
- [ ] 同步 Psycopg 连接在 FastAPI `def` 路由中的线程与连接生命周期
- [ ] 外键列为什么不自动建索引，以及何时应为外键补索引
- [ ] 如何用集成测试验证 `yield` 依赖在异常路径也会关闭连接

---

## 已知问题

- 当前 API 仍直接操作数据库，尚未拆分 Service / Repository。
- 当日没有自动化测试；CRUD 和错误路径主要依赖手工请求验证。
- `localhost` 被替换为 `127.0.0.1` 是本机环境修复，后续应由配置提供正确地址，而不是在数据库层修改 URL。

---

## 明日任务

- 学习 SQLAlchemy Core 的 Engine、Connection、`text()` 和命名参数。
- 用 Core 重写 Agent 查询，并观察生成或执行的 SQL。
- 完成 CTE、窗口函数实验，复习 JOIN、参数化 SQL 和事务提交行为。
