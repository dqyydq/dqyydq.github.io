...
| 问题 | 答案 |
|------|------|
| PG 多久检测到死锁？ | 等待超过 `deadlock_timeout` 后触发检测；常见默认值是 **1s**，以 `SHOW deadlock_timeout` 为准 |
| 谁被终止？ | 本次实验中 B 被终止；不要据此推导为“总是选择代价更小的事务” |
...
| 问题 | 原因 | 处理或后续验证 |
|------|------|----------------|
| `idempotency_key` 允许省略但模型原为 NOT NULL | API Schema 与数据库模型不一致 | 新增迁移 `1691672c5ae9` 改为 nullable |
| 普通 SELECT 看不到旧版本 | MVCC 可见性会过滤旧 tuple | 使用 `pageinspect` 在实验表上观察物理页 |
| 两个事务互相等待 | 资源加锁顺序相反 | 统一按主键顺序加锁，并准备对死锁错误做有限重试 |
| VACUUM 后空间没变小 | VACUUM 只标记空间可复用，不归还磁盘 | 评估 `VACUUM FULL` 或 `pg_repack` |

---

## 仍不理解的内容

- [ ] HOT Update（Heap-Only Tuple）原理
- [ ] 并行 VACUUM（PostgreSQL 13+）
- [ ] VACUUM FULL 实操与 pg_repack
- [ ] 复制槽对 VACUUM 的阻塞机制
- [ ] `idempotency_key` 冲突的 `IntegrityError` 处理

---

## 已知问题

- Run 创建采用“先查询再插入”，并发请求仍可能越过应用层检查；必须依靠数据库约束并正确处理 `IntegrityError`。
- 当前唯一约束是全局 `idempotency_key`，与项目建议的 `(conversation_id, idempotency_key)` 不一致。
- Run 状态转换尚无集中状态机或数据库 CHECK，非法状态和非法跳转仍可能写入。
- `app/api/runs.py` 在 Router 内直接提交事务，尚未形成 Service 决定事务边界的分层。
- Run API、事务回滚和幂等场景没有自动化测试。
- Day 7 没有独立 SQL 脚本保存 MVCC、死锁和 VACUUM 实验，复现实验目前依赖本笔记。

---

## 明日任务

- 先用两个 psql 会话验证 `FOR UPDATE SKIP LOCKED`：一个事务锁住最早 queued Run，另一个事务跳过它领取下一条。
- 在同一事务中完成“选择任务 + queued → running”，明确 commit 前后锁的生命周期。
- 实现最小 Worker 领取函数和状态转换规则，不接真实模型。
- 增加并发测试，证明多个 Worker 不会领取同一个 Run。
- 复习行锁、Read Committed 快照、死锁预防、回滚和幂等约束。
