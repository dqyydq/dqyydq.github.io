...
这就是 **Unit of Work 模式**——Session 跟踪所有变更（新增、修改、删除），flush 时一次性计算最小 SQL 集合并执行。相比手动管理——不会漏改、不会重复、事务安全。
...
| 问题 | 原因 | 解决 |
|------|------|------|
| `session.delete(id)` 报错 | ORM delete 需要传对象，不是 id | `session.delete(session.get(Model, id))` |
| `scalars()` 返回空 | 忘了在 `select()` 后面加 `.where()` 或条件写错 | 打印 SQL 日志确认 |
| relationship 访问报 DetachedInstanceError | Session 已关闭，lazy load 无法执行 | 设 `expire_on_commit=False` 或提前 eager load |
...
Day 5 开始进入 Alembic 数据库迁移——用版本化的方式管理表结构变更，不再手写 `CREATE TABLE` 或 `ALTER TABLE`。至此，项目的数据库栈完整了：Psycopg（原生 SQL）→ Core（命名参数+连接池）→ ORM（对象映射）→ Alembic（迁移管理）。
